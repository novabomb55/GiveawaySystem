...
(function() {
	...
	//Custom command by Nom
		/**
		  * @commandpath amount - Shows amount of tickets a user has
		  */
		if (command.equalsIgnoreCase('amount') || command.equalsIgnoreCase('amounts'))
		{
			if (tags.getTags().containsKey('subscriber') || tags.getTags().get('subscriber').equals('1')) 	
			{	
				$.say($.whisperPrefix(sender) + $.lang.get('ticketrafflesystem.ticket.usage', getTickets(sender))/subTMulti); //come back to this
			}
			else
			{
